    ...
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 mt-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg sm:rounded-lg">
            <div class="p-6 space-y-2" style="min-height: 300px;"> {{-- Adjusted minimum height --}}
                <h2 class="text-3xl font-bold text-gray-900 dark:text-white mb-4">Create Shopping List</h2> {{-- Reduced bottom margin --}}
                ...
                <form action="{{ route('shoppinglist.store') }}" method="POST" class="space-y-2"> {{-- Reduced spacing for form elements --}}
                    @csrf
                                ...
                                <ul class="text-sm text-gray-700 dark:text-gray-200">
                                    @foreach($products as $product)
                                        <li class="flex items-center p-2 hover:bg-gray-200 dark:hover:bg-gray-600 transition duration-200">
                                            <input id="product-{{ $product->id }}" type="checkbox" name="product_ids[]" value="{{ $product->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded dark:bg-gray-600 dark:border-gray-500" {{ in_array($product->id, old('product_ids', [])) ? 'checked' : '' }}>
